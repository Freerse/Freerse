...
class RelayTracker {
  ...
  Future<void> analyzeNostrEvent(event, SocketControl socketControl) async {
    ...
    if (eventMap["kind"] == 3) {
      ...
      if (eventMap["content"].isEmpty) {
        return;
      }
      // own adv
      Map content = {};
      try{
        content = jsonDecode(eventMap["content"]);
      }catch(e){
        print("parse error ${eventMap}");
      }
    ...
    if (eventMap["kind"] == 0) {
      ...
      trackRelays(pubkey, result["relays"], RelayTrackerAdvType.nip05, eventMap["created_at"]);
    }
    // by tag
    if (eventMap["kind"] == 1) {
      /* EXAMPLE
